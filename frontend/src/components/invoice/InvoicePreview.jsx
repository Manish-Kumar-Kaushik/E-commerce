...
const formatDate = (date) => {
  if (!date) return "—";
  const parsed = parseInvoiceDate(date);
  if (!parsed) return "—";
...
const InvoicePreview = ({
  ...
  const defaultData = {
    ...
    paymentMethod: '',
    placeOfSupply: '',
    billingAddress: { name: '', phone: '', street: '', city: '', state: '', zip: '', country: '' },
    items: [],
    subOrders: [],
    pricing: { subtotal: subtotal || 0, gstTotal: gstTotal || 0, couponDiscount: 0, platformDiscount: 0, shipping: 0, handling: 0, total: totalAmount || 0 },
    transactionId: '',
    gateway: '',
    refundStatus: '',
  }
  ...
  // prefer explicit items prop, else fall back to draft.items
  data.items = (Array.isArray(items) && items.length) ? items : (Array.isArray(draft.items) ? draft.items : defaultData.items)
  data.subOrders = vendorSplits?.length ? vendorSplits : (draft.vendorSplits || defaultData.subOrders)

  const copyId = () => {
    navigator.clipboard.writeText(data.invoiceNumber || data.orderNumber || '')
    setCopied(true)
    setTimeout(() => setCopied(false), 2000)
  }

  return (
    <div ref={contentRef} className="bg-slate-50 min-h-screen p-4 md:p-8 font-sans text-slate-800">
                ...
                <tbody className="divide-y divide-slate-100">
                  {data.items.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="px-4 py-4 text-right">{item.gstAmount ? formatCurrency(item.gstAmount) : `${item.gst}%`}</td>
          ...
          <div className="space-y-3">
             ...
             <div className="grid grid-cols-2 md:grid-cols-5 gap-4 p-4 border border-slate-200 rounded-xl bg-white shadow-sm">
                {[
                  ...
                  { icon: CalendarDays, label: "Paid At", val: formatDate(data.paidAt || data.orderDate) },
